...
fb.core.util.Path = goog.defineClass(null, {
  ...
  equals: function(other) {
    ...
    for (var i = this.pieceNum_,
        j = other.pieceNum_; i <= this.pieces_.length; i++, j++) {
    ...
    setTimeout(function() {
      self.cb(self.targetOrigin, d["d"], function(response, forceKeepWindowOpen) {
        ...
        self.doPost_({
          "a": "response",
          "d": response,
          "forceKeepWindowOpen": forceKeepWindowOpen
        });
      });
    }, 0);
  }
...
fb.login.transports.Popup.prototype.open = function(url, params, cb) {
  var self = this,
      isIE = fb.login.util.environment.isModernIE(),
      iframe,
      messageTarget;
  ...
  var req = fb.util.json.stringify({
    "a": "request",
    "d": params
  });
...
if (typeof NODE_CLIENT !== "undefined" && NODE_CLIENT) {
  ...
  FirebaseIFrameScriptHolder.prototype.doNodeLongPoll = function(url, loadCB) {
    var self = this;
    FirebaseIFrameScriptHolder.nodeRestRequest({
      url: url,
      forever: true
    }, function(body) {
      self.evalBody(body);

...
- You include it in the `directives` array of the `@Component` you want to use it in (only if your ionic version is below RC0).
...
@Component({
  ...
  directives: [MyDir] // <-- Don't forget me! (only if your ionic version is below RC0)
})
class MyPage {}
```
